 ...
 * @author yangna
 */
public class CommonChartViewBackup extends FrameLayout implements OnChartValueSelectedListener {
    ...
    public CommonChartViewBackup(Context context) {
        super(context);
        init(context, null, 0);
    }

    public CommonChartViewBackup(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public CommonChartViewBackup(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initAttr(AttributeSet attrs, int defStyle) {
        ...
        mLabelTextSize = a.getDimension(R.styleable.CommonChartView_labelTextSize, 14);
        mValueTextSize = a.getDimension(R.styleable.CommonChartView_valueTextSize, 14);
...
//        yAxis.setGranularity(1f);//禁止放大后标签重绘
    ...
    public void setXAxisLabelRotation(List<String> xValue) {
        ...
        if (currentChart == null) return;
        if (xValue.get(0).contains("师") || xValue.get(0).contains("兵直")) {
            currentChart.getXAxis().setLabelRotationAngle(-30);
        }
    }
